...
GRAPHQL_PATHS = [
    "/graphql", "/graphql/", "/api/graphql", "/api/graphql/", "/v1/graphql", "/v2/graphql","/api/graphiql/","/api/graphiql", 
...
class GraphQLScanner:
    ...
    def check_version_exposure(self):
        print("[*] Fingerprinting GraphQL Engine & Version...")
        version_info = []
        ...
        try:
            ...
            version_headers = {
                'X-Powered-By': 'Generic Power Header',
                'Server': 'Server Header',
                'X-Apollo-Server-Version': 'Apollo Server',
                'X-Hasura-Version': 'Hasura',
                'X-GraphQL-Version': 'GraphQL Generic'
            }
            
            for h, desc in version_headers.items():
                if headers.get(h):
                    version_info.append(f"[Header] {desc}: {headers.get(h)}")
            
            # Header Fingerprinting
            if 'x-apollo-tracing' in headers:
                version_info.append("[Header] Detected: Apollo Engine (Tracing Header)")
                
        except Exception:
            pass

        # --- 2. Error Message Fingerprinting ---
        # Engines often leak their identity in error messages even if introspection is off
        print("[*] Probing for Engine Signatures (Error Analysis)...")
        try:
            # Query a non-existent field to trigger an error
            probe_query = '{ _engine_probe_check }'
            data, status = self.send_query(probe_query)
            
            if data and 'errors' in data:
                err_str = json.dumps(data['errors'])
                
                # Apollo Detection
                if 'APOLLO' in err_str.upper() or 'apollo' in err_str:
                    version_info.append("[Fingerprint] Engine: Apollo Server (Detected in Error JSON)")

                # Graphene (Python) Detection
                if 'Graphene' in err_str or 'graphene' in err_str:
                     version_info.append("[Fingerprint] Engine: Graphene (Python)")

                # Hasura Detection
                if 'hasura' in err_str.lower():
                     version_info.append("[Fingerprint] Engine: Hasura GraphQL Engine")

                # GQLGen (Go) Detection
                if 'gqlgen' in err_str.lower():
                     version_info.append("[Fingerprint] Engine: GQLGen (Go)")

                # Hot Chocolate (.NET) Detection
                if 'HotChocolate' in err_str:
                     version_info.append("[Fingerprint] Engine: Hot Chocolate (.NET)")
                
                # Caliban (Scala) Detection
                if 'caliban' in err_str.lower():
                     version_info.append("[Fingerprint] Engine: Caliban (Scala)")

                # Check for specific error extensions often used by specific engines
                if data['errors'][0].get('extensions'):
                    ext = data['errors'][0]['extensions']
                    if 'code' in ext:
                        # Apollo often uses specific error codes
                        if ext['code'] == 'INTERNAL_SERVER_ERROR' or ext['code'] == 'GRAPHQL_VALIDATION_FAILED':
                             # This is a loose heuristic, often Apollo
                             pass 
                    if 'exception' in ext:
                         # Often Node.js based servers (Apollo/Express)
                         version_info.append("[Fingerprint] Engine: Likely Node.js based (Error Stack Trace present)")

        except Exception:
            pass

        # --- 3. Schema-Based Fingerprinting (If Introspection enabled) ---
        if self.schema:
            type_names = [t['name'] for t in self.schema['types']]
            
            # Hasura
            if any("hasura" in t.lower() for t in type_names):
                version_info.append("[Schema] Engine: Hasura GraphQL Engine")
            
            # Apollo Federation
            if any(t in type_names for t in ['_Service', '_Entity', 'federation']):
                version_info.append("[Schema] Engine: Apollo Federation")

            # Graphene
            if any("graphene" in t.lower() for t in type_names):
                 version_info.append("[Schema] Engine: Graphene (Python)")

            # AWS AppSync
            if any("appsync" in t.lower() for t in type_names):
                 version_info.append("[Schema] Engine: AWS AppSync")

            # --- 4. Querying common version fields found in Schema ---
            if self.schema.get('queryType') and self.schema['queryType'].get('fields'):
                fields = {f['name'] for f in self.schema['queryType']['fields']}
                
                # Common fields that might expose version
                targets = ['version', 'build', 'info', 'release', 'serviceinfo', 'systemstatus']
                found_targets = [f for f in targets if f in fields]
                ...
                for field in found_targets:
                    try:
                        ...
                        if d and 'data' in d and d['data'].get(field):
                            val = d['data'][field]
                            if isinstance(val, (dict, list)):
                                val = json.dumps(val)
                            version_info.append(f"[Query] Exposed Field '{field}': {val}")
                    except:
                        pass

        # --- 5. Brute Force Common Version Fields (If Schema unknown) ---
        if not self.schema:
            print("[*] Brute-forcing version fields...")
            common_version_fields = ['version', 'build', 'info', 'release', 'systemstatus']
            for field in common_version_fields:
                q = f'query {{ {field} }}'
                d, _ = self.send_query(q)
                if d and 'data' in d and d['data'].get(field):
                    val = d['data'][field]
        ...
        if version_info:
            ...
            self.findings.append({
                "vulnerability": "GraphQL Engine/Version Exposure",
                "severity": "Low",
                "description": "The server leaks engine type or version information.",
                "exposed_data": "\n".join(version_info)
            })
        else:
            print("[-] Could not determine specific GraphQL engine version.")

    def run(self):
        print(f"\n=== Scanning {self.url} ===")
        self.check_introspection()
        self.check_version_exposure() 
