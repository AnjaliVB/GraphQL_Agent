...
def is_graphql_json(response):
    """
    STRICT CHECK: Eliminates false positives by enforcing JSON structure.
    Returns True ONLY if response is valid JSON with 'data' or 'errors'.
    """
    try:
        # Force JSON parsing
        data = response.json()
        # Check for GraphQL structure keys
...
def validate_endpoint_strict(url):
    ...
    try:
        ...
        # (Many endpoints return 400 for missing headers/params but still identify as GraphQL)
...
def test_endpoint_loose(url):
    """
    Discovery check. Fast but must verify JSON to avoid false positives.
    """
    headers = {'Content-Type': 'application/json', 'User-Agent': HEADERS['User-Agent']}
    try:
