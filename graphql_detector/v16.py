...
class GraphQLScanner:
    ...
    def send_query(self, query, variables=None):
        ...
        try:
            # Faster timeout for scanning phase
...
def validate_endpoint_strict(url, allow_private=False):
    ...
    # OPTIMIZATION: Fast timeout for validation
    try:
...
def discover_endpoint_bruteforce(base_url, allow_private=False):
    ...
    found = []
    # OPTIMIZATION: Increased workers for parallel requests
...
def crawl_website(start_url, allow_private=False):
    ...
    # We limit depth logically: Fetch HTML -> Extract Links/JS -> Fetch JS -> Extract Endpoints
    # We don't need a recursive loop, just 2 layers of fetching.
    
    # Layer 1: Fetch the main page
    futures = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        # Submit initial URL
        futures[executor.submit(session.get, start_url, timeout=3, verify=False, allow_redirects=True)] = start_url
        
        # Process Layer 1 (HTML)
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                # Check if the start_url itself is an endpoint
                if 'json' in content_type:
                     if test_endpoint_loose(url, allow_private): found_endpoints.add(url)
                
                # If HTML, extract JS files and API calls
                if 'text/html' in content_type:
                    # Extract JS files and potential endpoints from HTML content
                    js_files = re.findall(r'src=[\"\'](.*?\.js.*?)[\"\']', response.text, re.IGNORECASE)
                    html_endpoints = extract_endpoints_from_text(response.text, url)
                    
                    # Immediately test endpoints found in HTML
                    for ep in html_endpoints:
                        if test_endpoint_loose(ep, allow_private): found_endpoints.add(ep)
                    
                    # Prepare Layer 2: Submit JS files for fetching
                    for js_path in js_files:
                        full_url = urljoin(url, js_path)
                        if full_url not in visited:
                             # We reuse the executor logic below or just add to a new list for simplicity
                             # Here we just read content directly if small, or fetch async
                             try:
                                 # Quick fetch for JS files
...
if __name__ == "__main__":
    ...
    if is_valid:
        ...
        # Run Crawler and Brute-forcer
        # Note: We run them sequentially, but internally they are parallel
