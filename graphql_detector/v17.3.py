...
class GraphQLScanner:
    ...
    def check_version_exposure(self):
        ...
        # 4. Brute-force common version fields if introspection failed or didn't find them
        if not any("Exposed Version Field" in v for v in version_info):
            print("[*] Brute-forcing version fields...")
            common_version_fields = ['version', 'build', 'info', 'release', 'systemstatus', 'getStatus']
            for field in common_version_fields:
                ...
                if d and 'data' in d and d['data'].get(field):
                    ...
                    if isinstance(val, (dict, list)):
                        val = json.dumps(val)
                    version_info.append(f"Exposed Version Field '{field}': {val}")
