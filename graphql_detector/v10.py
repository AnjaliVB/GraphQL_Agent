    ...
    def check_introspection(self):
        ...
        if data and 'data' in data and '__schema' in data['data']:
            ...
            self.findings.append({
                ...
                "exposed_data": f"Found {len(exposed_types)} exposed types (e.g., {', '.join(exposed_types[:5])})."
            })
        else:
            ...
            for test_id in test_ids:
                ...
                if data and 'errors' not in data and 'data' in data:
                    ...
                    if data['data'][result_key] is not None:
                        ...
                        # Try to fetch common sensitive fields
...
def validate_endpoint_strict(url):
    """Strict validation used to confirm if input is a real endpoint."""
    headers = {'Content-Type': 'application/json', 'User-Agent': HEADERS['User-Agent']}
    try:
        # Try a minimal valid query
        ...
        # Authentication errors usually mean the endpoint exists but is protected
        if r.status_code in [401, 403, 405]:
             return True, f"Potential Endpoint (Auth Required - {r.status_code})"
