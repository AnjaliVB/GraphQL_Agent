...
class GraphQLScanner:
    ...
    def send_query(self, query, variables=None):
        ...
        try:
            ...
            # Fallback if JSON parsing fails but response exists
            try:
    ...
    def check_introspection(self):
        ...
        # Check if dictionary and has schema
...
def check_graphql_signatures(text):
    """Checks text for common GraphQL error signatures."""
    text = text.lower()
    signatures = [
        "graphql", 
...
def validate_endpoint_strict(url):
    """
    Smart validation. Handles 400/500 errors if they contain GraphQL signatures.
    """
    ...
    try:
        ...
        # Case B: 400 / 500 Error (Check for GraphQL signatures)
        if r.status_code in [400, 500]:
            if check_graphql_signatures(r.text):
                return True, f"Valid (GraphQL Error via Status {r.status_code})"

        # Case C: Auth Required
        if r.status_code in [401, 403, 405]:
             return True, f"Found (Auth Required - {r.status_code})"
             
    except: pass

    # Test 2: Empty POST (Error based detection)
    try:
        r = requests.post(url, json={}, headers=headers, timeout=10, verify=False)
        if check_graphql_signatures(r.text):
            return True, "Valid (Error-Based Detection)"
    except: pass

    return False, "Invalid"

def test_endpoint_loose(url):
    """Loose check for discovery phase."""
    headers = {'Content-Type': 'application/json', 'User-Agent': HEADERS['User-Agent']}
    try:
        ...
        # Critical change: Check signatures even on non-200
        if check_graphql_signatures(r.text): return True
    except: pass
    return False
...
if __name__ == "__main__":
    ...
    if is_valid:
        ...
        print("[-] Input URL is not a direct GraphQL endpoint. Starting discovery...")
